...
SUITE["Cl2"]["fat_dot"] = @benchmarkable begin
    CliffordAlgebras.fatdotprod(a, b)
end setup=(cl2 = CliffordAlgebra(2); e1 = cl2.e1; e2 = cl2.e2; a = 2.0*e1 + 3.0*e2; b = 4.0*e1 + 5.0*e2)

SUITE["Cl2"]["scalar_product"] = @benchmarkable begin
    CliffordAlgebras.scalarprod(a, b)
end setup=(cl2 = CliffordAlgebra(2); e1 = cl2.e1; e2 = cl2.e2; a = 2.0*e1 + 3.0*e2; b = 4.0*e1 + 5.0*e2)
...
SUITE["Cl3"]["dot_product"] = @benchmarkable begin
    CliffordAlgebras.fatdotprod(e1 + e2, e2 + e3)
...
end setup=(CA = typeof(CliffordAlgebra(4)); mt = CliffordAlgebras.multtable(CA))

SUITE["Internals"]["multtable_cached_Cl5"] = @benchmarkable begin
    CliffordAlgebras.multtable(CA)
end setup=(CA = typeof(CliffordAlgebra(5)); mt = CliffordAlgebras.multtable(CA))
...
SUITE["PGA3D"]["gp_general"] = @benchmarkable begin
    a * b
end setup=(pga = CliffordAlgebra(:PGA3D); e1 = basevector(pga,1); e2 = basevector(pga,2); e3 = basevector(pga,3); a = 1 + e1 + 2*e2 + 3*e3 + (e1 ∧ e2); b = 2 + 3*e2 + e3 + 2*(e1 ∧ e3))

SUITE["PGA3D"]["sandwich"] = @benchmarkable begin
    R ≀ v
end setup=(pga = CliffordAlgebra(:PGA3D); e1 = basevector(pga,1); e2 = basevector(pga,2); v = e1 + 2*e2; B = 0.1*(e1 ∧ e2); R = exp(B))
...
SUITE["CGA3D"]["gp_vectors"] = @benchmarkable begin
    v1 * v2
end setup=(cga = CliffordAlgebra(:CGA3D); e1 = basevector(cga,1); e2 = basevector(cga,2); e3 = basevector(cga,3); e4 = basevector(cga,4); e5 = basevector(cga,5); v1 = e1 + 2*e2 + 3*e3 + e4 + e5; v2 = 2*e1 - e2 + 0.5*e3 + e4 - e5)

SUITE["CGA3D"]["bivector_exp"] = @benchmarkable begin
    exp(B)
end setup=(cga = CliffordAlgebra(:CGA3D); e1 = basevector(cga,1); e2 = basevector(cga,2); e3 = basevector(cga,3); B = 0.05*((e1 ∧ e2) + (e2 ∧ e3) + (e3 ∧ e1)))

SUITE["CGA3D"]["matrix_conversion"] = @benchmarkable begin
    matrix(a)
end setup=(cga = CliffordAlgebra(:CGA3D); e1 = basevector(cga,1); e2 = basevector(cga,2); e3 = basevector(cga,3); a = 1 + e1 + 2*e2 + 3*e3 + (e1 ∧ e2) + (e2 ∧ e3))
