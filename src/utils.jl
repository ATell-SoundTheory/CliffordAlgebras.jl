...
function adaptbasefordual(base)
    N = length(base)
    Nh = N ÷ 2
    Tuple( n > Nh && length(base[n])>=2 && levicivita([base[N-n+1]...,base[n]...])<0  
        ? Tuple( k==length(base[n]) 
            ? base[n][k-1] 
            : k+1==length(base[n]) 
                ? base[n][k+1] 
                : base[n][k] for k in 1:length(base[n])
        ) 
        : base[n] for n in 1:N
    )
end
...
function findpermutation(a,b)
    ...
    [findfirst(isequal(a[n]),b) for n in 1:length(a)]
end
