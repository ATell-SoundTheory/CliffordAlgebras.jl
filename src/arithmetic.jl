...
@generated function (+)(a::MultiVector{CA,Ta}, b::MultiVector{CA,Tb}) where {CA,Ta,Tb}
    d = dimension(CA)
    acc_a = [Int(0) for n in 1:d]
    acc_b = [Int(0) for n in 1:d]
    bi_a = baseindices(a)
    bi_b = baseindices(b)
    ...
    coeffs = ( iszero(acc_a[n]) ? :(b.c[$(acc_b[n])]) : iszero(acc_b[n]) ? :(a.c[$(acc_a[n])]) : :(a.c[$(acc_a[n])]+b.c[$(acc_b[n])]) for n in BI )
...
leftcontractionfilter(leftgrade, rightgrade, productgrade) = productgrade == rightgrade - leftgrade
rightcontractionfilter(leftgrade, rightgrade, productgrade) = productgrade == leftgrade - rightgrade
fatdotfilter(leftgrade, rightgrade, productgrade) = productgrade == abs(leftgrade - rightgrade)
scalarfilter(leftgrade, rightgrade, productgrade) = iszero(productgrade)

@generated function filteredprod(a::MultiVector{CA,Ta}, b::MultiVector{CA,Tb}, ::Val{Filter}) where {CA,Ta,Tb,Filter}
    d = dimension(CA)
    mt = multtable(CA)
    acc = [NTuple{3,Int}[] for n in 1:d]
    for (ka,base_a) in enumerate(baseindices(a)), (kb,base_b) in enumerate(baseindices(b))
        baseidx,coeff = mt[base_a][base_b]
        if !iszero(coeff) && Filter(basegrade(CA,base_a),basegrade(CA,base_b),basegrade(CA,baseidx))
    ...
    coeffs = (Expr(:call,:+,( :($(tup[3])*a.c[$(tup[1])]*b.c[$(tup[2])]) for tup in acc[n] )...) for n in BI)
    coeffsexpr = Expr(:call,:(NTuple{$K,$T}),Expr(:call,:tuple,coeffs...))
    :(MultiVector(algebra(a),$BI,$coeffsexpr))    
end
...
"""
(∧)(a::MultiVector{CA},b::MultiVector{CA}) where {CA} = filteredprod(a,b,Val(exteriorfilter))
...
"""
(⋅)(a::MultiVector{CA},b::MultiVector{CA}) where {CA} = filteredprod(a,b,Val(fatdotfilter))
...
"""
(⨼)(a::MultiVector{CA},b::MultiVector{CA}) where {CA} = filteredprod(a,b,Val(leftcontractionfilter))
...
"""
(⨽)(a::MultiVector{CA},b::MultiVector{CA}) where {CA} = filteredprod(a,b,Val(rightcontractionfilter))
...
"""
(⋆)(a::MultiVector{CA},b::MultiVector{CA}) where {CA} = filteredprod(a,b,Val(scalarfilter))
...
@generated function symprod(a::MultiVector{CA,Ta},b::MultiVector{CA,Tb},::Val{P}) where {CA,Ta,Tb,P}
    d = dimension(CA)
    mt = multtable(CA)
    acc = [NTuple{3,Int}[] for n in 1:d]
    for (ka,base_a) in enumerate(baseindices(a)), (kb,base_b) in enumerate(baseindices(b))
...
"""
(×₋)(a::MultiVector{CA,Ta},b::MultiVector{CA,Tb}) where {CA,Ta,Tb} = inv(promote_type(Ta,Tb)(2)) * symprod(a,b,Val(-1))
...
@generated function sandwichproduct(a::MultiVector{CA,Ta}, b::MultiVector{CA,Tb}) where {CA,Ta,Tb}
    # a * b * reverse(a)
    d = dimension(CA)
    mt = multtable(CA)
    acc = [NTuple{4,Int}[] for n in 1:d]
    for (ka,base_a) in enumerate(baseindices(a)), (kb,base_b) in enumerate(baseindices(b)), (ka_rev,base_a_rev) in enumerate(baseindices(a))
    ...
    coeffs = (Expr(:call,:+,( :($(tup[4])*a.c[$(tup[1])]*b.c[$(tup[2])]*a.c[$(tup[3])]) for tup in acc[n] )...) for n in BI)
    coeffsexpr = Expr(:call,:(NTuple{$K,$T}),Expr(:call,:tuple,coeffs...))
    :(MultiVector(algebra(a),$BI,$coeffsexpr))  
end
...
@generated function (Λᵏ)(mv::MultiVector, ::Val{k} ) where k
    CA = algebra(mv)
    indexbounds = 1 .+ (0,cumsum(ntuple(i->binomial(order(CA),i-1),1+order(CA)))...)
...
@generated function inv(mv::MultiVector)
    ...
    if all(isone,reversesigns) || all(s->isone(-s), reversesigns)
        ...
        BIone = Tuple(sort(unique(skipmissing(Iterators.flatten([iszero(mt[r][c][2]) ? missing : mt[r][c][1] for r in BI, c in BIinv])))))
        ea = [ Expr(:call,:+) for r in 1:length(BIone), c in 1:length(BIinv) ]
    ...
    products = Tuple(
        (i1,i2,mt[b1][b2][1],mt[b1][b2][2]) 
         for (i1,b1) in enumerate(BI), 
             (i2,b2) in enumerate(BI) 
         if !iszero(mt[b1][b2][2]) && isone(basereverse(CA,mt[b1][b2][1])
        )
    )
    ...
    for (i1,i2,bres,c) in products
        ...
        push!(coeffexpr[findfirst(isequal(bres),BIsa)].args, :($(c*rc)*mv.c[$i1]*mv.c[$i2]))
    end
    K = length(BIsa)
...
@generated function exp(mv::MultiVector)
    ...
    for ncset in ncsets
        if length(ncset) == 1 && ncset[1] == 1
            ...
            crosstermscancel = all(iszero(mt[i][j][2]+mt[j][i][2]) for i in ncset, j in ncset if i<j)
